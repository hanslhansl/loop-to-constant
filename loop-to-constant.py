...
def standard(a, b, c, d, q, p, y):
    r = 0
    r2 = 0
    ldict = locals()
    exec(python_string, globals(), ldict)
    return ldict["r"], ldict["r2"]
    
def resolved(a, b, c, d, q, p, y):
    ldict = locals()
    exec(resolved_string, globals(), ldict)
    return ldict["r"], ldict["r2"]


param = [random.randint(0, 1000) for _ in range(7)]
param[1] = param[0] + random.randint(0, 1000)
print("\nparam:", param)
print("standard:", standard(*param))
print("resolved:", resolved(*param))
assert standard(*param) == resolved(*param), "results do not match"


# mypy --namespace-packages --explicit-package-bases --disallow-any-generics --disallow-untyped-defs "F:\Paul\Computer\Programmieren\C++\Haupt-Projektmappe\PythonApplication\loop_resolve.py"
# pyright "F:\Paul\Computer\Programmieren\C++\Haupt-Projektmappe\PythonApplication\loop_resolve.py"
